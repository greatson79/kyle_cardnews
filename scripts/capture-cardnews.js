/**
 * ★동명 주의 (2026-08-13): 이 파일은 **비율 4:5 (기준 1080×1350 · 출력 2160×2700 2x 권장)** 다.
 *   내부 저장소 weekly-works 의 `src/scripts/capture-cardnews.js` 는 **같은 이름이지만 1:1(1080×1080) 구 스펙**이다.
 *   ★이름으로 판단하지 말 것 — 규격은 파일 내용으로 확인한다. (COO 판정 2026-08-13)
 */
#!/usr/bin/env node
/**
 * 카드뉴스 HTML → 개별 슬라이드 PNG 캡쳐 스크립트
 * 사용법: node scripts/capture-cardnews.js <slide-preview.html 경로>
 *
 * 출력: 같은 폴더에 slide-1.png, slide-2.png, ... 개별 파일 생성
 * 크기: 1080×1350 (4:5 — .slide 요소 540×675px × deviceScaleFactor 2)
 */
...
if (!htmlPath) {
  console.error('사용법: node scripts/capture-cardnews.js <slide-preview.html 경로>');
  process.exit(1);
}
...
(async () => {
  ...
  // 1080×1350 출력을 위해 540×675 뷰포트 + 2배 스케일
